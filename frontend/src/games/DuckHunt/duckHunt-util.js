  ...
  move(deltaTime) {
    ...
    if (
      !inBounds(
        ...
        { yMax: innerHeight, xMax: innerWidth, xMin: -100, yMin: -100 }
      )
    ) {
      this.respawn();
      this.escapedCount++;
...
export class Dog {
  ...
  idle({ alive }) {
    ...
    switch (this.birds.length) {
      ...
      case 2:
      case 3:
        return this.toState('show' + this.birds.length);
      default:
        return this.toState('show4+');
    }
