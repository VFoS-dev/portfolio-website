...
export class Matraex extends Component {
    ...
    render() {
        ...
        const { name, description, link, dates, stack, keyFeatures, parent } = projectData[focus];
        const [links, about] = this.createlinks(link);
        return (
            <Fragment>
                <center style={{ margin: "4% 0px", padding: '0px 15%' }}>
                    {parent && <Fragment>
                        <h4>This project is a client project for <a href={`/projects/${parent}`}>{projectData[parent].name}</a></h4>
