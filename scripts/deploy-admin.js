...
const BUCKET_NAME = process.env.AWS_ADMIN_BUCKET;
const AWS_REGION = process.env.AWS_REGION || 'us-west-2';
const CLOUDFRONT_DISTRIBUTION_ID = process.env.AWS_ADMIN_CLOUDFRONT_DISTRIBUTION_ID;

if (!BUCKET_NAME) {
  console.error('Error: AWS_ADMIN_BUCKET environment variable is required');
  process.exit(1);
}
...
async function deploy() {
  ...
  console.log(`Deploying admin to S3 bucket: ${BUCKET_NAME}`);
  ...
  */
  ...
  console.log(`\n✓ Deployment complete!`);
  console.log(`Admin panel available at: https://${BUCKET_NAME}`);
  ...
  if (CLOUDFRONT_DISTRIBUTION_ID) {
    ...
    try {
      const invalidationCommand = new CreateInvalidationCommand({
        ...
        InvalidationBatch: {
          CallerReference: `admin-deploy-${Date.now()}`,
          Paths: {
            Quantity: 1,
            Items: ['/*'],
          },
    ...
    console.log('\nNote: AWS_ADMIN_CLOUDFRONT_DISTRIBUTION_ID not set, skipping CloudFront invalidation');
  }
}
